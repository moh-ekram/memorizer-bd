...
export default function StatsDashboard({ 
  ...
  const activeCourse = allCourses.find(c => c.id.trim().toLowerCase() === activeCourseId?.trim().toLowerCase()) || allCourses[0];
      ...
      >
        ...
        <AnimatePresence>
          {showFlashcardCoverAnim && (
            <motion.div
              ...
              transition={{ 
                duration: settings?.flashcardBannerDurationSec ?? 3.5, 
                times: [0, 0.7, 1],
                ease: [0.22, 1, 0.36, 1] 
              }}
              onAnimationComplete={() => setShowFlashcardCoverAnim(false)}
              className="absolute inset-0 z-30 bg-gradient-to-br from-indigo-900 via-slate-900 to-indigo-950 rounded-3xl p-4 sm:p-6 md:p-8 flex items-center justify-center shadow-2xl border-2 border-indigo-400/40 pointer-events-none"
            >
              <div className="flex flex-col items-center justify-center text-center gap-4 sm:gap-5 bg-white/10 backdrop-blur-xl px-6 sm:px-10 py-5 sm:py-7 rounded-3xl border border-white/20 shadow-2xl text-white max-w-md w-full">
                
                {/* 3 Rotating Stacked Flashcards (One Front, One Back revolving) */}
                <div className="relative w-28 h-20 sm:w-36 sm:h-24 flex items-center justify-center my-1">
                  {/* Card 1 (Indigo/Blue Gradient) */}
                  <motion.div
                    animate={{
                      rotate: [-16, 0, 16, -16],
                      scale: [0.8, 1.08, 0.85, 0.8],
                      x: [-24, 0, 24, -24],
                      y: [4, -4, 4, 4],
                      zIndex: [10, 30, 20, 10],
                    ...
                    className="absolute inset-0 rounded-2xl bg-gradient-to-tr from-indigo-500 to-indigo-600 text-white p-2.5 sm:p-3 shadow-lg border border-indigo-300/40 flex flex-col justify-between"
                  >
                    <div className="flex items-center justify-between text-[8px] sm:text-[9px] font-bold opacity-80">
                      <span>Vocabulary</span>
                      <span>#01</span>
                    </div>
                    <div className="text-center font-extrabold text-xs sm:text-sm tracking-wide">
                      Flashcard
                    </div>
                  </motion.div>
                  ...
                  <motion.div
                    animate={{
                      rotate: [0, 16, -16, 0],
                      scale: [1.08, 0.85, 0.8, 1.08],
                      x: [0, 24, -24, 0],
                      y: [-4, 4, 4, -4],
                      zIndex: [30, 20, 10, 30],
                      opacity: [1, 0.85, 0.75, 1]
                    }}
                    transition={{
                      duration: 2.4,
                      repeat: Infinity,
                    ...
                    className="absolute inset-0 rounded-2xl bg-gradient-to-tr from-emerald-500 to-teal-600 text-white p-2.5 sm:p-3 shadow-xl border border-emerald-300/40 flex flex-col justify-between"
                  >
                    <div className="flex items-center justify-between text-[8px] sm:text-[9px] font-bold opacity-80">
                      <span>GRE / BCS</span>
                      <span>#02</span>
                  ...
                  {/* Card 3 (Amber/Orange Gradient) */}
                  <motion.div
                    animate={{
                      rotate: [16, -16, 0, 16],
                      scale: [0.85, 0.8, 1.08, 0.85],
                      x: [24, -24, 0, 24],
                      y: [4, 4, -4, 4],
                      zIndex: [20, 10, 30, 20],
                      opacity: [0.85, 0.75, 1, 0.85]
                    }}
                    transition={{
                      duration: 2.4,
                      repeat: Infinity,
                    ...
                    className="absolute inset-0 rounded-2xl bg-gradient-to-tr from-amber-500 to-orange-600 text-white p-2.5 sm:p-3 shadow-lg border border-amber-300/40 flex flex-col justify-between"
                  >
                    <div className="flex items-center justify-between text-[8px] sm:text-[9px] font-bold opacity-80">
                      <span>Memorize</span>
                      <span>100%</span>
                ...
                {/* Center-aligned Title and Line Below */}
                <div className="text-center flex flex-col items-center justify-center space-y-1">
                  <h3 className="text-xl sm:text-2xl md:text-3xl font-black text-white tracking-tight flex items-center justify-center gap-2">
                    <span>Flashcard Practice</span>
                    <ChevronRight className="w-5 h-5 text-indigo-200" />
                  </h3>
                  <p className="text-xs sm:text-sm text-indigo-200 font-bold uppercase tracking-wider text-center">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4 md:gap-6">
          <div className="space-y-1 min-h-[64px] flex flex-col justify-center">
            <div style={{ perspective: 1000 }} className="overflow-hidden">
              <AnimatePresence mode="wait">
                <motion.div
                  key={headerToggle ? 'flashcard-line' : 'course-line'}
                ...
                >
                  <h2 className="text-xl sm:text-2xl md:text-3xl font-black font-poppins tracking-tight text-white flex items-center gap-2">
                    {headerToggle ? (
                      ...
                      <span className="flex items-center gap-2 bg-gradient-to-r from-emerald-300 via-teal-200 to-amber-200 bg-clip-text text-transparent">
                        <BookOpen className="w-6 h-6 text-emerald-400" />
                        <span>{activeCourse ? activeCourse.title : "Vocabulary Mastery"}</span>
                      </span>
                    )}
                  </h2>
                </motion.div>
            ...
            <p className="text-xs sm:text-sm font-extrabold flex items-center gap-2 text-emerald-300 pt-0.5">
              <span className="inline-block w-2.5 h-2.5 rounded-full bg-emerald-400 animate-ping"></span>
              <span className="font-bold">
                Active Course: <span className="text-white font-black">{activeCourse ? activeCourse.title : 'Selected Course'}</span>
              </span>
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="space-y-1">
            <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
              ...
              Vocabulary Groups ({uniqueGroups.length > 0 ? `${uniqueGroups[0]}-${uniqueGroups[uniqueGroups.length - 1]}` : ''})
            </h3>
