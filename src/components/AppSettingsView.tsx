        ...
        {activeTab === 'account' && (
          <div className="space-y-4 font-['Poppins',sans-serif]">
            ...
            <div className="bg-white border border-slate-200/80 rounded-2xl p-4 sm:p-5 space-y-4 shadow-2xs">
              ...
              <div className="flex items-center justify-between gap-3 bg-indigo-50/50 border border-indigo-100 rounded-xl px-3.5 py-3">
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-slate-500 font-medium mt-0.5 leading-snug">
                    {settings.autoCloudBackup !== false
                      ? 'ON — every rating is saved to your account instantly.'
                      : 'OFF — changes stay on this device until you press Backup Now.'}
                  </p>
                </div>
                <button
                  type="button"
                  role="switch"
                  aria-checked={settings.autoCloudBackup !== false}
                  onClick={handleToggleAutoBackup}
                  className={`relative w-11 h-6 rounded-full transition-colors shrink-0 cursor-pointer border ${
                    settings.autoCloudBackup !== false ? 'bg-indigo-600 border-indigo-700' : 'bg-slate-200 border-slate-300'
                  }`}
                >
                  <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow-md transition-all ${settings.autoCloudBackup !== false ? 'left-[22px]' : 'left-0.5'}`} />
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50/80 border border-slate-100 p-3 rounded-xl">
                  <span className="block text-[10px] font-semibold uppercase tracking-wider text-slate-400">Connection</span>
                  <span className="block text-xs font-bold text-slate-800 mt-0.5 flex items-center gap-1.5">
                ...
                <div className="bg-slate-50/80 border border-slate-100 p-3 rounded-xl">
                  <span className="block text-[10px] font-semibold uppercase tracking-wider text-slate-400">Last Backup</span>
                  <span className="block text-xs font-bold text-slate-800 mt-0.5">
                    {lastCloudSyncAt ? formatLogTime(lastCloudSyncAt) : (syncStatus === 'synced' ? 'Just now' : 'Not yet')}
                  </span>
                </div>
                ...
                <div className="bg-slate-50/80 border border-slate-100 p-3 rounded-xl">
                  <span className="block text-[10px] font-semibold uppercase tracking-wider text-slate-400">Backed-up Items</span>
                  <span className="block text-xs font-bold text-slate-800 mt-0.5">
                    {Object.keys(progress || {}).length} studied + {Object.keys(settings || {}).length ? 'prefs' : ''}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-2.5">
                {displayActivityLogs.length === 0 ? (
                  <div className="p-6 text-center text-slate-400 text-xs font-medium bg-slate-50 rounded-xl border border-dashed border-slate-200">
                    {syncLogFilter === 'all'
                      ? 'No sync activity yet — rate a few words or press "Backup Now". Every change is saved to the cloud automatically.'
